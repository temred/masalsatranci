...
export const Piece: React.FC<PieceProps> = ({ type, color }) => {
  ...
  // Renk Ayarları
  // Beyaz Taşlar: İçi Beyaz, Dış Çizgisi Koyu Gri (Netlik için)
  // Siyah (Mor) Taşlar: İçi Koyu Mor. Kontürü kaldırdık (gövde rengiyle eşitledik).
  // Böylece taşlar "karmaşık çizgiler" olmadan net bir silüet olarak görünür.
  const fill = isWhite ? '#FFFFFF' : '#581c87'; // White vs Purple-900
  
  // Siyah taşlarda stroke (çizgi) rengini fill (dolgu) rengiyle aynı yapıyoruz.
  // Eğer 'none' yaparsak, Şah'ın tepesindeki haç veya At'ın detayları gibi sadece çizgiden oluşan kısımlar kaybolur.
  // Aynı renkte yaparak bu detayları gövdeyle birleştiriyoruz.
  const stroke = isWhite ? '#334155' : fill; 
  ...
  const renderPath = () => {
    switch (type.toLowerCase()) {
      ...
      case 'k': // King (Şah)
        return (
          <g>
            <path d="M22.5 11.63V6" strokeLinecap="round" />
            <path d="M20 8h5" strokeLinecap="round" />
  ...
  return (
    <div className="w-full h-full flex items-center justify-center select-none cursor-pointer p-[10%]">
      <svg 
        viewBox="0 0 45 45" 
