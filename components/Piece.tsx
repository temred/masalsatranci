...
export const Piece: React.FC<PieceProps> = ({ type, color }) => {
  ...
  // Renk Ayarları
  // Beyaz Taşlar: İçi Beyaz, Dış Çizgisi Koyu Gri
  // Siyah (Mor) Taşlar: İçi Koyu Mor, Dış Çizgisi Beyaz (Her zeminde net okunması için)
  const fill = isWhite ? '#FFFFFF' : '#581c87'; // White vs Purple-900
  const stroke = isWhite ? '#334155' : '#FFFFFF'; // Slate-700 vs White
  const strokeWidth = '2';
