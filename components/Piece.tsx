...
export const Piece: React.FC<PieceProps> = ({ type, color }) => {
  ...
  // RENK PALETİ
  // Beyaz Taşlar: İçi Beyaz, Çizgileri Koyu Gri
  const whiteFill = "#ffffff";
  const whiteStroke = "#334155"; // slate-700

  // Siyah Taşlar: İçi Koyu Mor, Çizgileri Açık Mor (Detayların görünmesi için)
  // Beyaz kontür yerine, gövdenin açık tonunu kullanıyoruz.
  const blackFill = "#581c87"; // purple-900
  const blackStroke = "#c084fc"; // purple-400 (Detayları ortaya çıkaran renk)

  const fill = isWhite ? whiteFill : blackFill;
  const stroke = isWhite ? whiteStroke : blackStroke;
  
  return (
    <div className="w-full h-full flex items-center justify-center select-none p-[12%] drop-shadow-md transition-transform hover:scale-110">
      <svg 
        viewBox="0 0 45 45" 
